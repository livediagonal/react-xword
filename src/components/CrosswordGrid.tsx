...
const CrosswordGrid: React.FC<CrosswordGridProps> = ({
  ...
  const handleCellClick = (row: number, col: number) => {
    ...
    const horizontalClueNumber =
      clueNumbers[horizontalStartRow][horizontalStartCol];
    const verticalClueNumber = clueNumbers[verticalStartRow][verticalStartCol];
  ...
  const handleKeyDown = (e: React.KeyboardEvent, row: number, col: number) => {
    ...
    if (e.key.length === 1 && /^[a-zA-Z0-9\-/:;()$&@".,?!']$/.test(e.key)) {
      ...
      if (clueOrientation !== "across") {
        ...
        const nextCell = findNextWhiteCell(
          grid,
          row,
          col,
        ...
        if (nextCell) {
          ...
          const clueNumber = findClueNumberForCell(
            nextRow,
            nextCol,
            "across",
            grid,
  ...
  const getCellClass = (row: number, col: number): string => {
    ...
    if (
      validatedCells &&
      validatedCells[row] &&
      validatedCells[row][col] !== undefined
    ) {
